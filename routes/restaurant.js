...
router.post('/edit_restaurant_images', upload.single('picture'), function (req, res, next) {
  ...
  try {
    var q = ""
    if (fieldname == 'Fssai') { q = "update restaurant set filefssai=? where restaurantid=?" }
    else if (fieldname == 'Shop Act') { q = "update restaurant set fileshopact=? where restaurantid=?" }
    else if (fieldname == 'Logo') { q = "update restaurant set filelogo=? where restaurantid=?" }
